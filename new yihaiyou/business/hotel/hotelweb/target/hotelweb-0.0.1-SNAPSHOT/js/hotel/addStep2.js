var addStep2 = {
	...
	doAddPriceDate : function() {
		...
		$.post(url, {proId: $("#productId").val()}, function(data) {
			if (data.isHas) {
				...
				addStep2.setEventSource(startDateStr,endDateStr,weekStr, member, marketPrice, cost, inventory);
			} else {
				show_msg(data.reMsg);
				return ;
			}
			...
			while (date <= endDate) {
				...
				if (tempDateMon != dateMon) {	// 超过当前月，跳出循环后继续遍历
					...
					if (weekStr.indexOf(dateDay) > -1) {	// 包含在选中的星期内
						...
						if (member) {	// 成人价
							data.push(addStep2.buildPrice(date, member));
						}
						if (marketPrice) {	// 成人价
							data.push(addStep2.buildMarketPrice(date, marketPrice));
						}
						if (cost){
							data.push(addStep2.buildCprice(date, cost));
						}
						if (inventory){
		...
		$('#editForm').form('submit', {
			...
			success : function(result) {
				...
				if(result.success==true){
					var url = "/hotel/hotel/addStep21.jhtml?productId="+result.productId+"#loctop";
	...
	doBackList : function() {
		...
		var url = "/hotel/hotel/addStep21.jhtml?productId="+productId+"#loctop";
		window.location.href = url;
	}
};
