...
var SailboatPriceType = {
    ...
    initEvt: function() {
        ...
        $('#addBookinInfoBtn').on('click', function(event) {
            ...
            if ($("#is_where").val() && $("#is_where").val() == "bg_platform") {
                SailboatPriceType.initFrameHeight();
            }
        });
        // add fee info
    ...
    getTicketPriceInfo: function() {
        ...
        $('#ticketPriceImgs').on('filebatchselected', function(event, files) {
            if ($("#is_where").val() && $("#is_where").val() == "bg_platform") {
                window.parent.$("#editIframe_0").css("height", $(".sailBaseMess").height());
            }
        });
    },
    doSavePriceInfo: function(validator, form, submitButton) {
        ...
        extraData["productId"] = $('#ticketId').val();

        var url = "";
        if ($("#is_where").val() && $("#is_where").val() == "bg_platform") {
            url = "/ticket/ticket/savePriceInfo.jhtml";
        } else {
                ...
                if (result.success) {
                    $.messager.show({
                        ...
                        afterClosed: function() {
                            ...
                            if ($("#is_where").val() && $("#is_where").val() == "bg_platform") {
                                window.parent.$("#show_type_dg").datagrid("load", {});
                                window.parent.$("#editPanel_0").dialog("close");
                            } else {
        ...
        $.each($('div.refund-info-item'), function (i, b) {
            ...
            if (isCheck) {
                ...
                if (contentNum > SailboatPriceType.RefundContentNumLimit) {
                    ...
                    $('body').animate({scrollTop: top}, 100);
                    $.messager.show({
                        msg: '退改说明内容字数过多(最多' + SailboatPriceType.RefundContentNumLimit + '字), 请删减!',
                        type: 'warn',
                        timeout: 2000
                    });
                    return false;
    ...
    initFrameHeight: function() {
        window.parent.$("#editIframe_0").css("height", $(".sailBaseMess").height() + 50);
    },
    doCancel: function() {
        if ($("#is_where").val() && $("#is_where").val() == "bg_platform") {
            window.parent.$("#show_type_dg").datagrid("load", {});
