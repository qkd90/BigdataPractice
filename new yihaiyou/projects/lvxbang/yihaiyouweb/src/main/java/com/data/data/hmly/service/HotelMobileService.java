...
import com.data.data.hmly.action.yihaiyou.request.PlanRecommendTripRequest;
...
//    private HotelRoomIndexService hotelRoomIndexService;
    ...
    public List<HotelSimpleResponse> listHotel(HotelSearchRequest hotelSearchRequest, Page page) throws ParseException {
        ...
        for (HotelSolrEntity entity : entities) {
            ...
            search.setEnd(end);
            List<PriceStatus> statuses = Lists.newArrayList(PriceStatus.GUARANTEE, PriceStatus.UP);
            List<HotelPrice> prices = hotelPriceService.list(search, statuses, new Page(1, 1), "price", "asc");
            if (prices.isEmpty()) {
    ...
    private void fillHotel(Long hotelId, Date enterDate, Date leaveDate, HotelSimpleResponse response) {
        ...
        List<PriceStatus> statuses = Lists.newArrayList(PriceStatus.GUARANTEE, PriceStatus.UP);
        List<HotelPrice> hotelPrice = hotelPriceService.list(priceFilter, statuses, new Page(1, 1), "price", "asc");
    ...
    public HotelResponse hotelDeatil(Hotel hotel, String startDate, String endDate) throws ParseException {
        ...
        List<PriceStatus> statuses = Lists.newArrayList(PriceStatus.GUARANTEE, PriceStatus.UP);
        List<HotelPrice> hotelPriceList = hotelPriceService.list(search, statuses, null, "price", "asc");
