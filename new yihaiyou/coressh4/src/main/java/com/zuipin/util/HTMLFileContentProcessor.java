...
public class HTMLFileContentProcessor implements TextFileContentProccessor {
    ...
    private String replaceLinks(String pattern, String rootURL, String content) {
        ...
        while (m1.find()) {
            url = m1.group();
            if (url.toLowerCase().indexOf("http://") < 0 && // http:// prefix
                                                            // not
                    // found
                    url.toLowerCase().indexOf("https://") < 0 || // https://
                                                                 // prefix not
                    // found
                    (url.toLowerCase().indexOf("?") > 0 && // http:// prefix
                                                           // found but in
                            // url parameters
                            url.toLowerCase().indexOf("http", url.toLowerCase().indexOf("?")) > 0 && url.substring(0, url.indexOf("?")).indexOf(
                            "http") < 0)) {
                ...
                p2 = Pattern.compile(tagName + "\\s*=\\s*[\"']?", Pattern.CASE_INSENSITIVE);
                m2 = p2.matcher(url);
                while (m2.find()) {
                    prefix = m2.group();
                    if (prefix.indexOf("\"") > 0) {
                    ...
                    if (url.indexOf(":/") > 0 || url.indexOf(":\\") > 0) { // absolute
                        ...
                        if (url.indexOf(":/") > 0) {
                            absoluteUrl = absoluteUrl.replace(
                                    absoluteUrl.substring(absoluteUrl.lastIndexOf(":") - 1, absoluteUrl.lastIndexOf("/") + 1), "");
                        } else {
                            absoluteUrl = absoluteUrl.replace(
                                    absoluteUrl.substring(absoluteUrl.lastIndexOf(":") - 1, absoluteUrl.lastIndexOf("\\") + 1), "");
                        }
                    ...
                    value = Pattern.compile(rootURL + "mailto", Pattern.CASE_INSENSITIVE).matcher(value).replaceAll("mailto");
                    break;
                }
            }
        }
