...
(function() {
    ...
    AreaRestriction._mapMoveendEvent = function(e) {
        // 如果当前完全没有出界，则无操作
    	if (contains(_bounds, _map.getBounds()))
    	{
    		return;
    	}
    ...
    {
    	var b1sw = bounds1.getSouthWest();
    	var b1ne = bounds1.getNorthEast();
    	var b2sw = bounds2.getSouthWest();
    	var b2ne = bounds2.getNorthEast();
    	
    	if (b2sw.lat < b1sw.lat || b2sw.lng < b1sw.lng)
    	{
    		return false;
    	}
    	if (b2ne.lat > b1ne.lat || b2ne.lng > b1ne.lng)
    	{
    		return false;
    	}
