...
var TrafficConstants = {
    ...
    getConstants : function(code, filter) {
        var resultArray = [];
        var codeArray = TrafficConstants[code];
...
var TrafficUtil = {
    ...
    getCodeName : function(codeType, code) {
        if (codeType) {
            var codeArray = TrafficConstants[codeType];
            for (var i = 0; i < codeArray.length; i++) {
    ...
    codeFmt : function(value, rowData, rowIndex) {
        if (value && this.codeType) {
            return TrafficUtil.getCodeName(this.codeType, value);
        }
        return '';
    },
    // 表格代码转换-是否
    ...
    dateTimeFmt : function(value, rowData, rowIndex) {
        ...
        return TrafficUtil.dateToString(date, this.datePattern);
    },
    ...
    dateNumToString : function(dateNum, pattern) {
        ...
        return TrafficUtil.dateToString(date, 'yyyy-MM-dd HH:mm:ss');
    },
    ...
    dateToString : function(date, pattern) {
        ...
        if ('yyyy-MM-dd HH:mm:ss' == pattern) {
            return o.y+'-'+TrafficUtil.lpad(o.M)+'-'+TrafficUtil.lpad(o.d)+' '+TrafficUtil.lpad(o.H)+':'+TrafficUtil.lpad(o.m)+':'+TrafficUtil.lpad(o.s);
        }
        if ('HH:mm:ss' == pattern) {
            return TrafficUtil.lpad(o.H)+':'+TrafficUtil.lpad(o.m)+':'+TrafficUtil.lpad(o.s);
        }
        if ('yyyy-MM-dd' == pattern) {
            return TrafficUtil.lpad(o.y)+'-'+TrafficUtil.lpad(o.M)+'-'+TrafficUtil.lpad(o.d);
        }
        if ('MM-dd' == pattern) {
            return TrafficUtil.lpad(o.M)+'-'+TrafficUtil.lpad(o.d);
        }
        if ('MM-dd HH:mm' == pattern) {
            return TrafficUtil.lpad(o.M)+'-'+TrafficUtil.lpad(o.d)+ " "+TrafficUtil.lpad(o.H)+':'+TrafficUtil.lpad(o.m);
        }
    },
    // 数字左侧补0
    lpad : function(num) {
