...
public class FerryAction extends FrameBaseAction {
    ...
    public Result saveTraffic() {
        if (traffic.getId() == null) {
            ...
            traffic.setTrafficType(TrafficType.SHIP);
...
//        String productId = (String) getParameter("productId");
    ...
    public Result list() {
        ...
        String leaveDateStr = (String) getParameter("leaveDate");
        if (traffic.getLeaveTransportation() != null) {
            System.out.println(traffic.getLeaveTransportation().getId());
        }
        Date nowDate = new Date();
        Date leaveDateStart = null;
        Date leaveDateEnd = DateUtils.getEndDay(nowDate, 0);

        if (StringUtils.isNotBlank(leaveDateStr)) {
            leaveDateStart = DateUtils.toDate(leaveDateStr);

            if (leaveDateStart.getTime() > nowDate.getTime()) {
                traffic.setLeaveDateStart(leaveDateStart);
            } else {
                traffic.setLeaveDateStart(nowDate);
                leaveDateStart = nowDate;
            }
        } else {
            traffic.setLeaveDateStart(nowDate);
            leaveDateStart = nowDate;
        }
        traffic.setLeaveDateEnd(leaveDateEnd);
        traffic.setProType(ProductType.ship);
        traffic.setTrafficType(TrafficType.SHIP);
        List<Traffic> traffics = trafficService.getValidlist(traffic, pageInfo, "updateTime");
        List<Traffic> newTrafficList = new ArrayList<Traffic>();
        for (Traffic trc : traffics) {
            trafficPriceService.findMinPriceAndInventory(trc, leaveDateStart);
            newTrafficList.add(trc);
        }
        ...
        return datagrid(newTrafficList, pageInfo.getTotalCount(), jsonConfig);
    }
