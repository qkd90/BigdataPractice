...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="/css/yhy/yhyHotel/homestay_tenant.css">
    <title>民宿/商户信息</title>
</head>
<body class="homestayTenant">
    <%@include file="../../yhy/yhyCommon/common_header.jsp" %>
	<input type="hidden" id="userId" value="${companyInfoData.userId}">
	<div class="roomset">
		<div class="accountMessage">
			...
			<ul class="account_detailUl accUl">
				...
				<li>密码：<span class="password" >********</span> <input type="hidden" model="password" id="passwordId" value=""> <span class="rewrite" id="changePassword">修改密码</span></li>
				<li>姓名：<span model="userName">${companyInfoData.userName}</span>
					<%--<span class="rewrite" id="changeName">--%>
						<%--<input id="userNameId" type="hidden" model="userName" value="${companyInfoData.userName}">修改姓名--%>
					<%--</span>--%>
				</li>
				<li>手机号：<span model="mobile">${companyInfoData.mobile}</span>
				<%--<span class="showL">绑定</span><span class="operateL">绑定</span>--%>
		...
		<div class="commpenyMessage">
			<div class="detail_header clearfix">
				...
				<s:if test="companyInfoData.status!=0">
					<span class="modify_message" onclick="HotelTenant.goCompanyEditInfo()">修改企业信息</span>
				</s:if>
			</div>
			<ul class="account_detailUl">
				...
				<%--<li class="idBox clearfix" id="contract_appendices">
					合同附件：
					<div style="float: right;float: right; position: relative; left: -275px;">
				...
				<s:iterator value="companyInfoData.appendiceses" var="appendices" status="q">
					<li><s:property value="#appendices.name"/>&nbsp;&nbsp;&nbsp;&nbsp;
						<s:if test="#appendices.type==@com.data.data.hmly.service.contract.entity.nums.ContractAppendiceType@image">
							<a href="javascript:void(0)" onclick="HotelTenant.openDownLoad('<%=com.zuipin.util.QiniuUtil.URL%><s:property value="#appendices.path"/>')">下载</a>&nbsp;&nbsp;
						</s:if>
						<s:else>
							<a href="<%=com.zuipin.util.QiniuUtil.URL%><s:property value="#appendices.path"/>">下载</a>&nbsp;&nbsp;
						</s:else>
						<%--<a href="javascript:void(0)" onclick="HotelTenant.openDownLoad('<%=com.zuipin.util.QiniuUtil.URL%><s:property value="#appendices.path"/>', '<s:property value="#appendices.name"/>')">下载</a>--%>
					</li>
				</s:iterator>
			</ul>
		</div>
			...
			<ul class="account_detailUl accUl">
				...
				<li><s:property value="#qualification.name"/>&nbsp;&nbsp;&nbsp;&nbsp;
					<s:if test="#qualification.type==@com.data.data.hmly.service.entity.DoucementType@image">
						<a href="javascript:void(0)" onclick="HotelTenant.openDownLoad('<%=com.zuipin.util.QiniuUtil.URL%><s:property value="#qualification.path"/>')">下载</a>&nbsp;&nbsp;
					</s:if>
					<s:else>
						<a href="<%=com.zuipin.util.QiniuUtil.URL%><s:property value="#qualification.path"/>">下载</a>&nbsp;&nbsp;
					</s:else>
					<%--<a href="javascript:void(0)" onclick="HotelTenant.openDownLoad('<%=com.zuipin.util.QiniuUtil.URL%><s:property value="#qualification.path"/>', '<s:property value="#qualification.name"/>')">下载</a>--%>
				</li>
				</s:iterator>
			</ul>
		</div>
		<%--<div class="accountMessage">--%>
			...
			<%--<ul class="account_detailUl accUl">--%>
				<%--<s:iterator value="companyInfoData.qualificationList" var="qualification" status="q">--%>
					<%--<li><s:property value="#qualification.name"/>&nbsp;&nbsp;<a href="<s:property value="#qualification.path"/>">下载</a></li>--%>
				<%--</s:iterator>--%>
			<%--</ul>--%>
		<%--</div>--%>
	</div>
						 ...
						 <tr style="height: 40px;">
							 ...
							 <td>
								 <div style="width:100%;">
									 ...
									 <div id="sendSms" class="getCode" style="width: 120px;float: right;" onclick="HotelTenant.getValidateMsgCode()">获取短信验证码</div>
								 </div>
							 </td>
						 </tr>
						 <tr style="height: 40px;">
...
<script type="application/javascript" src="/js/yhy/yhyHotel/homestay_tenant.js"></script>
</html>