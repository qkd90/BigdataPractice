/**
 * Created by zzl on 2016/3/11.
 */

//上传器对象
var commentimg_uploader = null;
// 文件总大小限制
var commentimg_max_total_fileSize = 0;
// 单个文件大小限制
var commentimg_max_singlel_fileSize = 0;
function createCommentImgUploader(options) {
    if (isNull(options)) {
        window.console.log('创建上传器时, 配置错误!请传入基本参数!')
        return;
    }
    //通用图片上传配置
     commentimg_uploader = WebUploader.create({
    ...
    // 单次最多文件数目
    //var max_file_num = commentimg_uploader.options.fileNumLimit;
    // 文件总大小限制
    commentimg_max_total_fileSize = WebUploader.Base.formatSize(commentimg_uploader.options.fileSizeLimit, 0, ['B', 'KB', 'MB']);
    // 单个文件大小限制
    commentimg_max_singlel_fileSize = WebUploader.Base.formatSize(commentimg_uploader.options.fileSingleSizeLimit, 0, ['B', 'KB', 'MB'])
    $("#upload_tip_a").html('').html(
        '本次最多可以上传' + commentimg_uploader.options.fileNumLimit + '张图片'
        + ', 图片选择完成后, 请点击开始上传!'
    )
    $("#upload_tip_b").html('').html(
        '单张图片不可超过' + commentimg_max_singlel_fileSize
        + ', 全部图片大小不可超过' + commentimg_max_total_fileSize
    );
    //TODO
    //bugs: 问题: commentimg_uploader.getStats().queueNum 队列中的文件数目为何一直为0 ???
    // 文件加入队列之前的操作 // 此处拦截超出数量限制的文件
    //commentimg_uploader.on('beforeFileQueued', function (file) {
    ...
    //});
    //当文件队列被加入时候
    commentimg_uploader.on('fileQueued', function (file) {
        //打开上传组件窗口
        ...
        //创建缩略图
        commentimg_uploader.makeThumb(file, function (error, src) {
            if (error) {
        ...
        $close.click(function () {
            // 从队列中移除该文件
            commentimg_uploader.removeFile(file, true);
    ...
    //当文件队列加载完成时候(所有文件!!)
    commentimg_uploader.on('filesQueued', function (files) {
        ...
        $(".Upload_xp").find("i.close").click(function () {
            ...
            //console.log(commentimg_uploader);
            $.each(files, function (i, file) {
                //console.log(file);
                if (commentimg_uploader != null) {
                    commentimg_uploader.removeFile(file, true);
                }
            });
            //console.log('队列中文件数目-关闭窗口后: ' + commentimg_uploader.getStats().queueNum);
            // 重置上传器 !!重要!!
            resetQueue();
            // 关闭上传窗口
            ...
            // 取消本次上传按钮事件, 避免重复绑定
            $("#start_upload_btn").unbind('click');
            //console.log(commentimg_uploader.getStats().queueNum);
        });
    });
    //某个文件开始上传前触发, 一个文件只会触发一次
    commentimg_uploader.on('uploadStart', function (file) {
        var $li = $("#f_" + file.id);
        var $info = $li.find("p");
        $info.html('').html('正在上传');
    });
    //文件上传过程中,进度条事件
    commentimg_uploader.on('uploadProgress', function (file, percentage) {
        var $li = $("#f_" + file.id);
    ...
    //当某个文件上传到服务器!成功时候
    commentimg_uploader.on('uploadSuccess', function (file, response) {
        var $li = $("#f_" + file.id);
        var $percent = $li.find("span");
        // 执行某张图片上传到七牛成功后的回调函数
        ...
        if (saveResult) {
            //获取已经成功上传的文件数目
            success_file_num = commentimg_uploader.getStats().successNum;
            // 上传总进度条累加
            ...
            commentimg_uploader.getStats().successNum -= 1;
    ...
    //当某个文件上传错误时候
    commentimg_uploader.on('uploadError', function (file, reason) {
        var $li = $("#f_" + file.id);
    ...
    //当某个文件上传完成时候, 不管是否成功
    commentimg_uploader.on('uploadComplete', function (file) {
        var $li = $("#f_" + file.id);
        $li.find("p").remove();
    });
    // 当所有文件上传结束时候
    commentimg_uploader.on('uploadFinished', function () {
    ...
    // 上传对象发生错误时候
    commentimg_uploader.on('error', function (code) {
        //console.log(code);
        var info = '';
        switch (code) {
            case 'Q_EXCEED_NUM_LIMIT' :
                info = '文件数量超过限制,本次最多' + commentimg_uploader.options.fileNumLimit + '张';
                break;
            case 'Q_EXCEED_SIZE_LIMIT' :
                info = '文件总大小不能超过' + commentimg_max_total_fileSize;
                break;
            case 'F_EXCEED_SIZE' :
                info = '单个文件大小不能超过' + commentimg_max_singlel_fileSize;
                break;
            case 'F_DUPLICATE' :
                info = '选择了重复的文件!';
                break;
    ...
    function resetQueue() {
        // 重置上传器, 同时清空各项计数
        if (commentimg_uploader != null) {
            //commentimg_uploader.destroy();
            //commentimg_uploader = null;
            commentimg_uploader.reset();
        }
        //console.log('重置后队列中文件数目: ' + commentimg_uploader.getStats().queueNum);
        //console.log('队列中文件数目-重置后: ' + commentimg_uploader.getStats().queueNum);
        //console.log('队列中上传成功的文件数-重置后: ' + commentimg_uploader.getStats().successNum);
        //console.log('队列中上传中的文件数-重置后: ' + commentimg_uploader.getStats().progressNum);
        //console.log('队列中被删除的文件数-重置后: ' + commentimg_uploader.getStats().cancelNum);
        //console.log('队列中无效的文件数-重置后: ' + commentimg_uploader.getStats().invalidNum);
        //console.log('队列中上传失败的文件数-重置后: ' + commentimg_uploader.getStats().uploadFailNum);
        //console.log('队列中被暂停的文件数-重置后: ' + commentimg_uploader.getStats().interruptNum);
        //总上传文件数目
        total_file_num = 0;
        //成功上传文件数目
        success_file_num = 0;
...
// 通用上传器初始化配置方法
function initCommentImgUploader(options) {
    if (isNull(options)) {
    ...
    // 创建上传器
    createCommentImgUploader(options);
