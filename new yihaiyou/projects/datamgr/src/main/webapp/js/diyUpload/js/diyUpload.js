...
(function ($) {
    ...
    $.fn.extend({
        ...
        diyUpload: function (opt, serverCallBack) {
            ...
            if (opt.fileNumLimit) {
                opt['fileNumLimit'] = opt.fileNumLimit;
            }
...
//            	console.info(file.name);
    ...
    function createBox($fileInput, file, webUploader) {
        ...
        var li = '<li id="fileBox_' + file_id + '" class="diyUploadHover"> \
					...
					<input type="hidden" class="diyInput" name="fileName" value="'+file.name+'"/>\
					<div class="diyBar"> \
        ...
        * var li = '<li id="fileBox_' + file_id + '" class="diyUploadHover"> \
         ...
         <div class="diyCancel"></div> \
         <div class="diySuccess"></div> \
         <textarea id="text_' + file_id + '" class="diyFileName" name="desc"  value="'+file.name+'" />\
         <input type="hidden" class="diyInput" name="fileName" value="'+file.name+'"/>\
         <div class="diyBar"> \
         <div class="diyProgress"></div> \
         <div class="diyProgressText">0%</div> \
...
//        $fileBox.find('.diySuccess').append('<input type="hidden" name="fileName" value="'+file.name+'"/>');
        //绑定取消事件;
...
function showImage($fileInput, address, id, canDelete) {
    var $parentFileBox = $fileInput.next('.parentFileBox');
    //添加父系容器;
    if ($parentFileBox.length <= 0) {
        var div = '<div class="parentFileBox"> \
    ...
    var li = '<li id="fileBox_' + id + '" class="diyUploadHover"> \
					...
					<input type="hidden" class="fileBoxId" value="'+address+'" ></input> \
				</li>';
    $parentFileBox.children('.fileBoxUl').append(li);
    //父容器宽度;
    var $width = $('.fileBoxUl>li').length * 180;
    ...
    var viewImage = address;
    if(viewImage.indexOf('qiniu') > -1  ){
    	if(viewImage.indexOf('?imageView') > -1){
    		viewImage = viewImage.split('?imageView')[0];
    	}
    	viewImage += '?imageView2/1/w/170/h/150/q/75';
    }
    $fileBox.find('.viewThumb').append('<img src="' + viewImage + '" />');
    //绑定取消事件;
    ...
    //绑定easyui tooltip
    $fileBox.find('.viewThumb').tooltip({
    	position:'top',
    	content:'<img src="'+address+'" width="500px" />',
    	showEvent:'mouseenter'
    });
}

function showImageAndEdit($fileInput, address, id, fileName, desc) {
    var $parentFileBox = $fileInput.next('.parentFileBox');
    //添加父系容器;
    if ($parentFileBox.length <= 0) {
        var div = '<div class="parentFileBox"> \
    ...
    var li = '<li id="fileBox_' + id + '" class="diyUploadHover"> \
					...
					<div class="diyCancel"></div> \
					<div class="diySuccess"></div> \
				    <textarea id="text_' + id + '" class="diyFileName" name="desc"  value="'+desc+'" />\
					<input type="hidden" class="diyInput" name="fileName" value="'+fileName+'"/>\
					<div class="diyBar"> \
							<div class="diyProgress"></div> \
							<div class="diyProgressText">0%</div> \
					</div> \
    ...
    $parentFileBox.width($width);
    var $fileBox = $parentFileBox.find('#fileBox_' + id);
    $fileBox.find('.viewThumb').append('<img src="' + address + '" >');
    $fileBox.children('.diyFileName').val(desc);
    //绑定取消事件;
    $fileBox.children('.diyCancel').one('click', function () {
        $(this).parent().remove();
    });
 ...
 * @param id
 */
function showImageWithoutCancel($fileInput, address, id) {
    var $parentFileBox = $fileInput.next('.parentFileBox');
    //添加父系容器;
    if ($parentFileBox.length <= 0) {
        var div = '<div class="parentFileBox"> \
