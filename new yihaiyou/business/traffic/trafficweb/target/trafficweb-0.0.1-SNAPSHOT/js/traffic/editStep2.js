var editStep2 = {
	...
	doAddPriceDate : function() {
		...
		editStep2.setEventSource(startDateStr,endDateStr,weekStr,price, marketPrice, cPrice, inventory, flightTime, leaveTimeStr, arriveTimeStr);
	},
	// 清除所有报价
			...
			while (date <= endDate) {
				...
				if (tempDateMon != dateMon) {	// 超过当前月，跳出循环后继续遍历
					...
					if (weekStr.indexOf(dateDay) > -1) {	// 包含在选中的星期内
						...
						if (leaveTimeStr) {
							data.push(editStep2.buildLeaveTime(date, leaveDate));
						}
						...
						if (arriveTimeStr) {
							data.push(editStep2.buildArriveTime(date, arriveDate));
						}

						if (price) {
							data.push(editStep2.buildPrice(date, price));
						}
						if (marketPrice) {
							data.push(editStep2.buildMarketPrice(date, marketPrice));
						}
						if (cPrice) {
							data.push(editStep2.buildCprice(date, cPrice));
						}
						if (inventory) {
		...
		$('#editForm').form('submit', {
			...
			success : function(result) {
				...
				if(result.success==true){
					var url = "/traffic/traffic/editStep21.jhtml?productId="+result.productId+"#loctop";
	...
	doBackList : function() {
		...
		var url = "/traffic/traffic/editStep21.jhtml?productId="+productId+"#loctop";
		window.location.href = url;
	}
};
