...
var mIndexModule = angular.module("mIndexModule", ['infinite-scroll']);
mIndexModule.controller('mIndexCtrl', function($scope, $http, $state, storage, Banner) {
    ...
    $scope.preCityCode = "";
    //$scope.currentCityCode = "";
    // 获取新app首页数据
    $http.post(LXB_URL.mIndexData, {}).success(function(result) {
        if (result.success) {
            $scope.result = result;
            // 初始化线路列表城市代码
    ...
    $scope.$watch("finish.over", function () {
        // 幻灯广告
        Banner.init({
            container: '#index-top-slider-container',
            effect: 'fade',
            autoplay: 3000,
            pagination: '#index-top-slide-page'
        });
    });
