 ...
 */
var mInternalTourIndexModule = angular.module('mInternalTourIndexModule', ['infinite-scroll']);
mInternalTourIndexModule.controller('mInternalTourIndexCtrl', function($scope, $http, Banner, TourInit) {
    ...
    $scope.$watch("finish.over", function () {
        // 幻灯广告
        Banner.init({
            container: '#iti-top-slider-container',
            effect: 'fade',
            autoplay: 3000,
            pagination: '#iti-top-slide-page'
        });
    });
    ...
    // 获取周边游首页数据
    $http.post(LXB_URL.mInternalTourIndexData, {}).success(function(result) {
        if (result.success) {
            $scope.result = result;
            // 初始化banner
            $scope.init();
    ...
    $scope.getAreaLineListData = function () {
        ...
        $http.post(LXB_URL.mIndexAreaLineData, {
            ...
            'lineSearchRequest.lineType': 'china',
