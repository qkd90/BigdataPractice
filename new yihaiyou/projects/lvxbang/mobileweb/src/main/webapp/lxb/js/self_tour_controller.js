...
var mSelfTourIndexModule = angular.module('mSelfTourIndexModule', ['infinite-scroll']);
mSelfTourIndexModule.controller('mSelfTourIndexCtrl', function($scope, $http, Banner, TourInit) {
    ...
    $scope.$watch("finish.over", function () {
        // 幻灯广告
        Banner.init({
            container: '#sti-top-slider-container',
            effect: 'fade',
            autoplay: 3000,
            pagination: '#sti-top-slide-page'
        });
    });
    ...
    // 获取自助游首页数据
    $http.post(LXB_URL.mSelfTourIndexData, {}).success(function(result) {
        if (result.success) {
            $scope.result = result;
            // 初始化banner
            $scope.init();
    ...
    $scope.getAreaLineListData = function () {
        ...
        $http.post(LXB_URL.mIndexAreaLineData, {
            ...
            'lineSearchRequest.productAttr': '自助游'
