...
function pickerDateRange(inputId, options) {
    ...
    var defaults = {
        ...
        noCurMonthDis: 'noCurMonthRangeDis', //
 ...
 * 上一个月方法
 */
pickerDateRange.prototype.preMonthFun = function(isCompare) {
    var __method = this;
...
pickerDateRange.prototype.nextMonthFun = function(isCompare) {
    var __method = this;
    __method.calendar_endDate.setMonth(__method.calendar_endDate.getMonth() + 1, 1);
    __method.mOpts.endDate = __method.date2ymd(__method.calendar_endDate).join('-');
    __method.init(isCompare);
...
pickerDateRange.prototype.init = function (isCompare) {
    ...
    // 上一个月
    if (__method.mOpts.usePreMonthFun) {
        $('#' + this.preMonth).bind('click', function () {
            __method.preMonthFun();
        });
    }

    // 下一个月
    if (__method.mOpts.useNextMonthFun) {
        $('#' + this.nextMonth).bind('click', function () {
            __method.nextMonthFun();
        });
    }
...
pickerDateRange.prototype.selectDate = function (ymd) {
    ...
    if (this.startDateId == this.dateInput) {
        ...
        // 如果结束时间小于开始时间, 重新选择开始日期 -zzl 修改于2016-08-15
...
pickerDateRange.prototype.show = function (isCompare, __method) {
    ...
    $("#" + this.calendarId).css('top', pos.top + (__method.mOpts.theme == 'ta' ? 45 : 22) + 'px');
    //第一次显示的时候，一定要初始化输入框
...
pickerDateRange.prototype.close = function (btnSubmit) {
    ...
    // 判断开始结束是否相同
    if ($('#' + this.startDateId).val() == $('#' + this.endDateId).val()) {
        return false;
    } else {
...
pickerDateRange.prototype.fillDate = function (year, month, index) {
    ...
    if (isTaTheme) {
        ...
        if (!this.mOpts.magicSelect) {
            if (0 == index && this.mOpts.useNextMonthFun) {
                $(td).append('<a href="javascript:void(0);" id="' + this.nextMonth + '"><i class="i_next"></i></a>');
            }
            // 如果是第一个月的日期，则加上上一个月的链接
            if (index + 1 == this.mOpts.calendars && this.mOpts.usePreMonthFun) {
    ...
    for (var d = dateBegin; d.getTime() <= dateEnd.getTime(); d.setDate(d.getDate() + 1)) {
        if (d.getTime() < firstDayOfMonth.getTime()) { // 当前月之前的日期
            tdClass = this.mOpts.theme + '_' + this.mOpts.noCurMonthDis;
            deviation = '-1';
            //console.log(deviation + "--" + d.getDate() + "--月份:" + d.getMonth() + 1);
        } else if (d.getTime() > lastDayOfMonth.getTime()) { // 当前月之后的日期
            tdClass = this.mOpts.theme + '_' + this.mOpts.noCurMonthDis;
            deviation = '1';
            //console.log(deviation + "--" + d.getDate() + "--月份:" + d.getMonth() + 1);
        } else if ((this.mOpts.stopToday == true && d.getTime() < today.getTime()) || d.getTime() < __method.mOpts.minValidDate || ('' !== __method.mOpts.maxValidDate && d.getTime() > __method.mOpts.maxValidDate)) {
