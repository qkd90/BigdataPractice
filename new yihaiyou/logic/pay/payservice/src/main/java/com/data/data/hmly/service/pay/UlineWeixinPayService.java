...
/**
 * Created by huangpeijie on 2017-02-16,0016.
 */
public class UlineWeixinPayService extends PayService {
    private static final Logger logger = Logger.getLogger(UlineWeixinPayService.class);
    ...
    public Map<String, Object> doWeixinOrder(Order order) {
        ...
        paramMap.put("time_start", DateUtils.format(order.getCreateTime(), "yyyyMMddHHmmss"));
        paramMap.put("time_expire", DateUtils.format(order.getWaitTime(), "yyyyMMddHHmmss"));
        paramMap.put("attach", order.getId() + "-" + order.getOrderType().name());
        paramMap.put("total_fee", totalFee + ""); //微信的支付金额的单位是“分”
        paramMap.put("spbill_create_ip", "110.84.38.253");
        paramMap.put("notify_url", propertiesManager.getString("ULINE_WEIXIN_NOTIFY_URL"));
        paramMap.put("trade_type", "NATIVE");
        paramMap.put("sign", getSign(paramMap));

        String xmlData = toXml(paramMap);

        Map<String, Object> map = postForXML(apiUrl + UlineCfg.WEIXIN_SAVE_ORDER, xmlData);
    ...
    public Map<String, Object> doWeixinOrder(FerryOrder order) {
        ...
        paramMap.put("time_start", DateUtils.format(order.getCreateTime(), "yyyyMMddHHmmss"));
        paramMap.put("time_expire", DateUtils.format(order.getWaitTime(), "yyyyMMddHHmmss"));
        paramMap.put("attach", order.getId() + "-" + OrderType.ferry.name());
        paramMap.put("total_fee", totalFee + ""); //微信的支付金额的单位是“分”
        paramMap.put("spbill_create_ip", "110.84.38.253");
        paramMap.put("notify_url", propertiesManager.getString("ULINE_WEIXIN_NOTIFY_URL"));
        paramMap.put("trade_type", "NATIVE");
...
//		payLog.setNotifyType(paramMap.get("notify_type").toString());
    ...
    public boolean doDealNotify(Map<String, String> orderMap, String sign, Order order) {
        ...
        if (tradeStatus.equals("SUCCESS")) {
            if (order.getStatus() == OrderStatus.WAIT) {
                order.setPayType(OrderPayType.ULINEWECHAT);
                doBusiness(order);
    ...
    public Map<String, Object> closeOrder(String orderNo) {
        ...
        paramMap.put("sign", getSign(paramMap));
        String xmlData = toXml(paramMap);
        Map<String, Object> map = postForXML(apiUrl + UlineCfg.WEIXIN_CLOSE_ORDER, xmlData);
        if ("SUCCESS".equals(map.get("return_code")) && "SUCCESS".equals(map.get("result_code"))) {
            map.put("status", "ok");
            return map;
        } else {
    ...
    public Map<String, Object> queryOrder(String orderNo) {
        ...
        paramMap.put("sign", getSign(paramMap));
        String xmlData = toXml(paramMap);
        Map<String, Object> map = postForXML(apiUrl + UlineCfg.WEIXIN_QUERY_ORDER, xmlData);
        if ("SUCCESS".equals(map.get("return_code")) && "SUCCESS".equals(map.get("result_code"))) {
            map.put("status", "ok");
            return map;
        } else {
    ...
    public RefundResult refundOrder(String orderNo, String refundNo, Float totalPrice, Float refundPrice) {
        ...
        paramMap.put("total_fee", Float.valueOf(totalPrice * 100).intValue() + "");
        paramMap.put("refund_fee", Float.valueOf(refundPrice * 100).intValue() + "");
        paramMap.put("op_user_id", mchId);
        paramMap.put("sign", getSign(paramMap));
        String xmlData = toXml(paramMap);
        Map<String, Object> map = postForXML(apiUrl + UlineCfg.WEIXIN_REFUND_ORDER, xmlData);
    ...
    /**
     * 退款订单查询
     * 查询时优先使用退款号，当退款号为空时使用订单号
     *
     * @param orderNo  订单号
     * @param refundNo 退款号
     * @return
     */
    public Map<String, Object> queryRefundOrder(String orderNo, String refundNo) {
        String nonceStr = com.taobao.util.UUIDUtil.getUUID();
        Map<String, String> paramMap = Maps.newTreeMap();
        paramMap.put("mch_id", mchId);
        paramMap.put("nonce_str", nonceStr);
        ...
        paramMap.put("sign", getSign(paramMap));
        String xmlData = toXml(paramMap);
        Map<String, Object> map = postForXML(apiUrl + UlineCfg.WEIXIN_QUERY_REFUND_ORDER, xmlData);
        if ("SUCCESS".equals(map.get("return_code")) && "SUCCESS".equals(map.get("result_code"))) {
            map.put("status", "ok");
            return map;
        } else {
