...
var TransportConstants = {
    ...
    // 线路描述图片目录
    transortDescImg:'transport/transport/',
    // 票型
    ...
    getConstants : function(code, filter) {
        var resultArray = [];
        var codeArray = TransportConstants[code];
...
var TransportUtil = {
    ...
    getCodeName : function(codeType, code) {
        if (codeType) {
            var codeArray = TransportConstants[codeType];
            for (var i = 0; i < codeArray.length; i++) {
    ...
    codeFmt : function(value, rowData, rowIndex) {
        if (value && this.codeType) {
            return TransportUtil.getCodeName(this.codeType, value);
        }
        return '';
    },
    // 表格代码转换-是否
    ...
    dateTimeFmt : function(value, rowData, rowIndex) {
        ...
        return TransportUtil.dateToString(date, this.datePattern);
    },
    ...
    dateNumToString : function(dateNum, pattern) {
        ...
        return TransportUtil.dateToString(date, 'yyyy-MM-dd HH:mm:ss');
    },
    ...
    dateToString : function(date, pattern) {
        ...
        if ('yyyy-MM-dd HH:mm:ss' == pattern) {
            return o.y+'-'+TransportUtil.lpad(o.M)+'-'+TransportUtil.lpad(o.d)+' '+TransportUtil.lpad(o.H)+':'+TransportUtil.lpad(o.m)+':'+TransportUtil.lpad(o.s);
        }
        if ('HH:mm:ss' == pattern) {
            return TransportUtil.lpad(o.H)+':'+TransportUtil.lpad(o.m)+':'+TransportUtil.lpad(o.s);
        }
        if ('yyyy-MM-dd' == pattern) {
            return TransportUtil.lpad(o.y)+'-'+TransportUtil.lpad(o.M)+'-'+TransportUtil.lpad(o.d);
        }
    },
    // 数字左侧补0
    lpad : function(num) {
