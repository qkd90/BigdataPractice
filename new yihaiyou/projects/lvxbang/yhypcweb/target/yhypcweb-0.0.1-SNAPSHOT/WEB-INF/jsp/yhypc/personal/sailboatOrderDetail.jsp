...
<head>
	...
	<link rel="stylesheet" href="/css/personal/personal_Sailorder.css">
	<title>海上休闲订单详情-一海游</title>
</head>
                    ...
                    <c:choose>
						...
						<c:when test="${orderAllVo.status == '预订成功'}">
							<div class="pay_commit" style="">
								...
								<div class="commitBtn commitComment" <c:if test="${orderAllVo.hasComment}">style="display: none;"</c:if> data-name="${orderAllVo.details[0].name}" data-cover="${orderAllVo.details[0].cover}" data-pro-id="${orderAllVo.details[0].proId}" data-price-id="${orderAllVo.details[0].priceId}" data-pro-type="sailboat" data-detail-id="${orderAllVo.details[0].id}">我要点评</div>
							...
							<c:choose>
								<c:when test="${orderAllVo.status == '待支付'}">
									<div class="pay_wait">
										...
										<div class="paybtn" id="diypay" data-pay-url="/yhypc/order/orderSailboatPay.jhtml?orderId=${orderAllVo.id}">立即支付</div>
									</div>
				...
				<div class="Messagelist">
					<div class="Messagecontain">
						<div class="order_part">
							<p class="listtitle"><i></i>游艇帆船订单信息</p>
							<p class="product"></p>
							<div class="hotelmes clearfix">
								<div class="le">
									<p><span class="l_title">票型名称：</span>${orderAllVo.seatType}</p>
...
<script type="text/javascript" src="/js/personal/personal_Sailorder.js"></script>
