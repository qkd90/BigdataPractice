...
public class MfwService {
    ...
    public static String getAdviceTimeFromApp(int mfwId) {
        ...
        String url = MfwOAuthService.getRequestUrl(String.format(POI_DETAIL_API, mfwId), HTTP_METHOD_GET, new HashMap<String, String>());
        String json = getResponse(url);
        PoiDetailResult result = new Gson().fromJson(json, PoiDetailResult.class);
        if (result.getData() == null)
            return null;
        return result.getData().getPoi().getRefer_time();
    }
...
//            System.out.println(url);
    ...
    public List<PoiListResult.DataEntity.ListEntity> getAllPoi(int areaId) {
        int offset = 1;
        List<PoiListResult.DataEntity.ListEntity> results = new ArrayList<PoiListResult.DataEntity.ListEntity>();
        do {
            PoiListResult scenicList = getPoiList(areaId, offset);
            if (scenicList.getData() == null) {
                break;
            }
            results.addAll(scenicList.getData().getList());
//            for (ScenicList.ResultEntity result : ) {
//            }
            if (scenicList.getData().getHas_more() == 0)
                break;
            offset += 15;
        } while (offset < 1000);
        return results;
