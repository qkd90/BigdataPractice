...
var ViewPriceCalendar = {
    ...
    initComm: function() {
        ...
        $('#savePriceBtn').on('click', function(event) {
            ViewPriceCalendar.saveHotelPriceCalendar();
        });
    ...
    setPriceData: function () {
        ...
        if (valuationModels === "commissionModel") {
            $('#costPrice').val(memberPrice - memberPrice * valuationValue / 100);
        ...
        ViewPriceCalendar.setEventSource(startDateStr, endDateStr, weekStr, memberPrice, costPrice, inventory);
    },
            ...
            while (date <= endDate) {
                ...
                if (tempDateMon != dateMon) {	// 超过当前月，跳出循环后继续遍历
                    ...
                    if (weekStr.indexOf(dateDay) > -1) {	// 包含在选中的星期内
                        if (member) {
                            data.push(ViewPriceCalendar.buildMemberPrice(date, member));
                        }
                        if (cost) {
                            data.push(ViewPriceCalendar.buildCostprice(date, cost));
                        }
                        if (inventory){
    ...
    saveHotelPriceCalendar: function() {
        ...
        formData['typePriceId'] = $('#hotelPriceId').val();
        $.ajax({
            url: '/hotel/hotel/saveTypePriceDate.jhtml',
            type: 'post',
            dataType: 'json',
            data: formData,
            success: function(result) {
