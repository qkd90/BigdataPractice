...
<body class="order_details_fj_wf order_details"><!-- #BeginLibraryItem "/lbi/head.lbi" -->
<!--Float_w-->
...
<div class="main cl">
    <div class="free_exercise_div cl">
        <p class="n_title">
            ...
            <%--<c:if test="${orderDetailResponse.trafficType == '火车票'}">--%>
                <%--火车--%>
            <%--</c:if>--%>
            <%--<c:if test="${orderDetailResponse.trafficType != '火车票'}">--%>
                <%--飞机--%>
        ...
        <div class="free_e_fl fl">
            <!--标题-->
            <div class="order_details_title return_traffic_title">
                <p class="type">订单状态：<span>${orderDetailResponse.printStatus}</span></p>
                <dl>
                    <dt>
            ...
            <div class="order_details_div order_detail_traffic">
                ...
                <div class="order_details_center">
                        ...
                        <c:when test="${orderDetailResponse.trafficType == '火车票'}">
                            <dl>
                                ...
                                <div class="w3 fl">
                                    ...
                                    ${orderDetailResponse.trafficCode}/去程</div>
                                <div class="w4 fl" style="text-align: left;margin-left: 30px;width:220px;" <c:if test="${empty orderDetailResponse.company}"> style="width: 211px;text-align: left;margin-left: 41px;"</c:if>>
                                    <p style="margin-top: 10px;line-height: 10px;">出发: <fmt:formatDate value="${orderDetailResponse.date}" pattern="yyyy-MM-dd HH:mm"/> ${orderDetailResponse.leavePort}</p><br/>
                        ...
                        <c:otherwise>
                            <dl>
                                ...
                                <div class="w3 fl">
                                    <%--<c:if test="${orderDetailResponse.company != null}">${orderDetailResponse.company}/</c:if>--%>
                                    ${orderDetailResponse.trafficCode}/去程</div>
                                <div class="w4 fl" style="text-align: left;" <c:if test="${orderDetailResponse.company != null}"> style="width:250px"</c:if>>
                                    <p style="margin-top: 10px;line-height: 10px;">起飞: <fmt:formatDate value="${orderDetailResponse.date}" pattern="yyyy-MM-dd HH:mm"/> ${orderDetailResponse.leavePort}</p><br/>
                    ...
                    <ul class="textL" style="<c:if test="${returnOrderDetailResponse.company == null}">padding-left:70px;</c:if><c:if test="${returnOrderDetailResponse.company != null}">padding-left:40px;</c:if>">
                        <c:if test="${orderDetailResponse.trafficType == '火车票'}">
                            <c:forEach items="${returnOrderDetailResponse.touristList}" var="tourist" varStatus="status">
                                <li>
                                    ...
                                    <div class="w3 fl" style="margin-left:53px;">身份证: ${tourist.idNumber}</div>
                                    <div class="w4 fl" style="    margin-left: 53px;">电话: ${tourist.tel}</div>
                                </li>
                            </c:forEach>
                        </c:if>
                        <c:if test="${orderDetailResponse.trafficType != '火车票'}">
                            <c:forEach items="${returnOrderDetailResponse.touristList}" var="tourist" varStatus="status">
                                <li>
                                    ...
                                    <div class="w2 fl" style="    margin-left: 25px;">姓名: ${tourist.name}</div>
                                    <div class="w3 fl" style="    margin-left: 30px;">身份证: ${tourist.idNumber}</div>
                    ...
                    <c:choose>
                        <c:when test="${returnOrderDetailResponse.trafficType == '火车票'}">
                            <dl>
                                <dt class="textC">
                                <div class="w1 fl disn"></div>
                                <div class="w2 fl disn"></div>
                                <div class="w3 fl">
                                    <c:if test="${returnOrderDetailResponse.company != null}">${returnOrderDetailResponse.company}/</c:if>${returnOrderDetailResponse.trafficCode}/返程</div>
                                <div class="w4 fl" style="text-align:left;margin-left: 30px;width:220px;" <c:if test="${empty returnOrderDetailResponse.company}"> style="width: 211px;text-align: left;margin-left: 41px;"</c:if>>
                                    <p style="margin-top: 10px;line-height: 10px;">出发: <fmt:formatDate value="${returnOrderDetailResponse.date}" pattern="yyyy-MM-dd HH:mm"/> ${returnOrderDetailResponse.leavePort}</p><br/>
                                    <p style="line-height: 10px;">到达: <fmt:formatDate value="${returnOrderDetailResponse.leaveDate}" pattern="yyyy-MM-dd HH:mm"/> ${returnOrderDetailResponse.arrivePort}</p>
                                </div>
                                <div class="w5 fl">${returnOrderDetailResponse.type}</div>
                                <div class="w6 fl">${returnOrderDetailResponse.price}</div>
                                <div class="w7 fl">${returnOrderDetailResponse.num}</div>
                                <div class="w8 fl">${returnOrderDetailResponse.singleCost}</div>
                                </dt>
                            </dl>
                        </c:when>
                        <c:otherwise>
                            <dl>
                                ...
                                <div class="w2 fl disn"></div>
                                <div class="w3 fl">
                                    <%--<c:if test="${returnOrderDetailResponse.company != null}">${returnOrderDetailResponse.company}/</c:if>--%>
                                    ${returnOrderDetailResponse.trafficCode}/返程</div>
                                <div class="w4 fl" style="text-align: left;" <c:if test="${returnOrderDetailResponse.company != null}"> style="width:250px"</c:if>>
                                    <p style="margin-top: 10px;line-height: 10px;">起飞: <fmt:formatDate value="${returnOrderDetailResponse.date}" pattern="yyyy-MM-dd HH:mm"/> ${returnOrderDetailResponse.leavePort}</p><br/>
                                    <p style="line-height: 10px;">落地: <fmt:formatDate value="${returnOrderDetailResponse.leaveDate}" pattern="yyyy-MM-dd HH:mm"/> ${returnOrderDetailResponse.arrivePort}</p>
                                </div>
                                <div class="w5 fl">${returnOrderDetailResponse.type}</div>
                                <div class="w6 fl">${returnOrderDetailResponse.price}</div>
                                <div class="w7 fl">${returnOrderDetailResponse.num}</div>
            ...
            <div class="nav">
                <div class="free_e_fr_div cl">
                    <c:if test="${orderDetailResponse.status != 'PAYED'}">
                        <a href="${INDEX_PATH}/lvxbang/order/orderReturnFlight.jhtml?orderId=${orderDetailResponse.id}"
                           class="oval4">修改订单</a>
                    </c:if>
