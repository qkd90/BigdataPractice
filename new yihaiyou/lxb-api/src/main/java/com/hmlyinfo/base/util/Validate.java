...
public class Validate {
     ...
     * 检查用户是否具有数据的操作权限
     *
     * @param o data to check
     */
    public static void dataAuthorityCheck(Object o) {
        try {
            String dataOwnerId = BeanUtils.getProperty(o, "userId") + "";
            if (MemberService.getCurrentUserId() != Long.parseLong(dataOwnerId)) {
                throw new BizValidateException(ErrorCode.ERROR_50002);
            }
        } catch (Exception e) {
            throw new BizValidateException(ErrorCode.ERROR_50002);
        }
