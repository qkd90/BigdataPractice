...
import com.data.data.hmly.action.yihaiyou.request.SimpleUser;
import com.data.data.hmly.action.yihaiyou.response.AccountLogResponse;
...
//            errMsg = "用户被锁定或者被删除";
    ...
    public Result updatePersonalPassword() {
        ...
        user.setPassword(MD5.caiBeiMD5(newPassword));
        memberService.update(user);
        result.put("success", true);
        return jsonResult(result);
    }
    ...
    @AjaxCheck
    @NeedLogin
    public Result updatePersonalPayPassword() {
        user = getLoginUser();
        user = memberService.get(user.getId());
        if (StringUtils.isNotBlank(user.getPayPassword())) {
            String pwd = MD5.caiBeiMD5(password);
            if (!pwd.equals(user.getPayPassword())) {
                result.put("success", false);
                result.put("errMsg", "密码错误");
                return jsonResult(result);
            }
        }
        user.setPayPassword(MD5.caiBeiMD5(newPassword));
        memberService.update(user);
        result.put("success", true);
        return jsonResult(result);
    }
...
//        }
    ...
    public Result balanceLog() throws LoginException {
        ...
        accountLog.setInStatus(statuses);
        List<AccountLog> accountLogList = accountLogService.findListByAccountLogAll(accountLog, "createTime", "desc");
