var editStep2 = {
	...
	init:function(){
        editStep2.dateStart = $('#dateStart').val();
		editStep2.initComp();
	...
	initComp:function(){
		...
		KindEditor.ready(function(K) {
			var quoteDescK = K.create('#quoteContainDescK', {
				...
				afterChange: function() {
					...
					if (hasNum > editStep2.limitNum) {
						//超过字数限制自动截取
						var strValue = this.text();
						strValue = strValue.substring(0, editStep2.limitNum);
						this.text(strValue);
						...
						//计算剩余字数
						$('textarea[name="quoteContainDesc"]').next().children('.green-bold').html(editStep2.limitNum-hasNum);
					}
				},
				afterBlur: function() {
					this.sync();
			...
			var quoteNoDescK = K.create('#quoteNoContainDescK', {
				...
				afterChange: function() {
					...
					if (hasNum > editStep2.limitNum) {
						...
						//计算剩余字数
						$('textarea[name="quoteNoContainDesc"]').next().children('.green-bold').html(editStep2.limitNum-hasNum);
					}
				},
				afterBlur: function() {
					this.sync();
			...
			var quoteOwnK = K.create('#quoteOwnK', {
				...
				afterChange: function() {
					...
					if (hasNum > editStep2.limitNum) {
						...
						//计算剩余字数
						$('textarea[name="quoteOwn"]').next().children('.green-bold').html(editStep2.limitNum-hasNum);
					}
				},
				afterBlur: function() {
					this.sync();
		...
		$('#calendar').fullCalendar({
            ...
            dayClick: function(date, jsEvent, view) {
                ...
                if (dateStr < editStep2.dateStart) {    // 选择的时间在开始时间之前
                    return ;
                } else {
	...
	doAddPriceDate : function() {
		...
		editStep2.setEventSource(startDateStr,endDateStr,weekStr,discountPrice,marketPrice,rebate,childPrice,childMarketPrice, childRebate,oasiaHotel,inventory);
	},
	// 清除所有报价
			...
			while (date <= endDate) {
				...
				if (tempDateMon != dateMon) {	// 超过当前月，跳出循环后继续遍历
					...
					if (weekStr.indexOf(dateDay) > -1) {	// 包含在选中的星期内
                        ...
                        var oasiaHotelData = editStep2.buildOasiaHotel(date, oasiaHotel);
						if (oasiaHotelData) {	// 单房差
							data.push(oasiaHotelData);
						}
                        var inventoryData = editStep2.buildInventory(date, inventory);
                        if (inventoryData) {	// 库存
		...
		$('#editForm').form('submit', {
			...
			onSubmit : function() {
				...
				if (!quoteContainDesc) {
					show_msg("费用包含说明不能为空");
					return false;
				} else if (quoteContainDesc.length > editStep2.maxLimitNum) {
				...
				if (!quoteNoContainDesc) {
					...
					//return false;
				} else if (quoteNoContainDesc.length > editStep2.maxLimitNum) {
                ...
                if (!quoteOwn) {
                    //show_msg("费用不包含说明不能为空");
                    //return false;
                } else if (quoteOwn.length > editStep2.maxLimitNum) {
...
//				$('#quoteDesc').val(quoteDesc);
    ...
    // 成人价
    buildDiscountPrice : function(date, discountPrice) {
        var data = null;
    ...
    buildOasiaHotel : function(date, oasiaHotel) {
        var data = null;
        if (oasiaHotel) {	// 单房差
            var vid = "7"+date.getTime();
            data = {
                id:vid,
                oasiaHotel:oasiaHotel,
                title:'g.单房差'+oasiaHotel,
                start:LineUtil.dateToString(date,'yyyy-MM-dd')
            };
        }
        return data;
    ...
    buildInventory : function(date, inventory) {
        var data = null;
        if (inventory) {	// 单房差
            var vid = "8"+date.getTime();
            data = {
                id:vid,
                inventory:inventory,
                title:'h.库存'+inventory,
                start:LineUtil.dateToString(date,'yyyy-MM-dd')
            };
        }
        return data;
