...
var FerryOrderPay = {
    ...
    init: function () {
        FerryOrderPay.bindEvent();
        FerryOrderPay.initDate();
        FerryOrderPay.timers();
    },


    initDate: function () {
        FerryOrderPay.orderId = $("#orderId").val();
        FerryOrderPay.detailUrl = "/yhypc/personal/ferryOrderDetail.jhtml?id=" + FerryOrderPay.orderId + "&type=ferry";
    },

    bindEvent: function () {
        FerryOrderPay.payway();
        FerryOrderPay.balancepay();
        FerryOrderPay.topay();
        //FerryOrderPay.paykey();
    },
    ...
    payway: function () {
                ...
                yes: function () {
                    $.post("/yhypc/ferry/cancelOrder.jhtml", {
                        ...
                        if (data.success) {
                            ...
                            location.href = FerryOrderPay.detailUrl;
                        } else {
                            $.message.alert({
                                title: "提示",
                                info: data.errMsg
        ...
        $(".payContain .next").on("click", function () {
            if (FerryOrderPay.payType == "wechat") {
                $.post("/yhypc/pay/wechatPay.jhtml", {
                    orderId: FerryOrderPay.orderId,
                    orderType: "ferry"
                }, function (data) {
                    if (data.success) {
                        FerryOrderPay.makeQrcode("wechat_qrcode", data.payForm);
            ...
            } else if (FerryOrderPay.payType == "zhifubao") {
                $.post("/yhypc/pay/aliPay.jhtml", {
                    orderId: FerryOrderPay.orderId,
                    orderType: "ferry"
                }, function (data) {
                    if (data.success) {
                        FerryOrderPay.makeQrcode("zhifubao_qrcode", data.payForm);
                ...
                location.href = "/yhypc/pay/cmbPay.jhtml?orderId=" + FerryOrderPay.orderId;
            }
        });
    },
            ...
            if ($(this).hasClass('balance')) {
                $.post("/yhypc/user/userInfo.jhtml", {}, function (data) {
                    if (data.success) {
                        ...
                        FerryOrderPay.paykey(scrolltop);
    ...
    timers: function () {
        ...
        setInterval(function () {
            $.post("/yhypc/ferry/orderStatus.jhtml", {
                orderId: FerryOrderPay.orderId
            }, function (data) {
                if (data.success) {
                    if (data.status != "WAIT") {
