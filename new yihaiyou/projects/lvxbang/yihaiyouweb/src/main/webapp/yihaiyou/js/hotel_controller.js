...
mHotelIndexModule.controller('mHotelIndexCtrl', function ($scope, $http, $state, Banner, storage, Position, DateUtils, HotelDate, $rootScope) {
    ...
    $scope.getHotelIndexData = function () {
        $http.post(yhyUrl.mHotelIndexData, {cityId: $scope.hotelCityCode}).success(function (result) {
            if (result.success) {
                $scope.result = result;
    ...
    // 双月日历, 日期范围跨月份, 切换下个月判断(日期范围不跨月份, 自动切换下个月, 反之, 不自动切换下一个月)
    ...
    $scope.nights = ($scope.dateRange[1].getTime() - $scope.dateRange[0].getTime()) / 86400000;
    ...
    $scope.showPriceCondition = function () {
        ...
        //console.log( $scope.hotelPriceCondition);
    };
