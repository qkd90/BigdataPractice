...
hotelListModule.controller("HotelListCtrl", function ($scope, $http, storage, DateUtils, HotelDate, $rootScope) {
    ...
    $scope.initStorage = function () {
        ...
        if (!storage.get("orderColumn")) {
            storage.set("orderColumn", "showOrder");
        }
    ...
    if (!dayRange || dayRange.length <= 0) {
        ...
        storage.set(yhyKey.hotelDetailRoomFilter, {tipInDate: startDate, tipOutDate: endDate});
    } else {
    ...
    $scope.hotelDateOptions = {
        ...
        success: function (data) {
            $scope.$apply(function () {
                ...
                storage.set(yhyKey.hotelDetailRoomFilter, {tipInDate: data.startDate, tipOutDate: data.endDate});
            });
        }
    };
    // 初始化日期控件
    ...
    if (storageHotelPriceList.length == 1) {
        $.each(initHotelPriceList, function (i, item) {
            if (storageHotelPriceList[0] == item.lowPrice && item.highPrice == 0) {
                item.priceClass = "on";
            } else {
                item.priceClass = "";
            }
