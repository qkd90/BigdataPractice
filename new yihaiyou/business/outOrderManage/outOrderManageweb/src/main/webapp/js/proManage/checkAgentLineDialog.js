...
var CheckAgentLineDialog = {

    init: function() {

        CheckAgentLineDialog.initPriceType();
    },

    initPriceType: function() {

        var url = "/proManage/productManage/getLineTypePriceList.jhtml?toplineId=" + $("#ipt_topline_id").val() + "&lineId=" + $("#ipt_line_id").val();
        ...
        $('#priceType_dg').datagrid({
            ...
            columns:[[
                ...
                {field:'adultRebate',title:'成人佣金',width:'100',
                    formatter: function(value, rowData, index) {
                        ...
                        var hipt = '<input type="text" id="adult_'+ index + '_'+ rowData.id +'" onkeyup="CheckAgentLineDialog.getChangeValue('+index+','+rowData.id+')" style="width: 95px;height: 22px;display:none;" value="'+rowData.adultRebate+'" temp-value="0">';
                ...
                {field:'childRebate',title:'儿童佣金',width:'100',
                    formatter: function(value, rowData, index) {
                        ...
                        var hipt = '<input type="text" id="child_'+ index + '_' + rowData.id +'" onkeyup="CheckAgentLineDialog.getChangeValue('+index+','+rowData.id+')" style="width: 95px;height: 22px;display:none;" value="'+rowData.childRebate+'" temp-value="0">';
            ...
            onLoadSuccess:function(data){
                ...
                if (checkRows.length == data.rows.length) {
                    $("#btn_agentLine").linkbutton("disable");
                }
            },
    ...
    getChangeValue: function(index, id) {
        ...
        if (!CheckAgentLineDialog.checkRate(childRebate)) {
            $("#child_"+index+"_"+id+"").val("");
        }
        if (!CheckAgentLineDialog.checkRate(adultRebate)) {
