 ...
 */
var mOverseasTourIndexModule = angular.module('mOverseasTourIndexModule', ['infinite-scroll']);
mOverseasTourIndexModule.controller('mOverseasTourIndexCtrl', function($scope, $http, Banner, TourInit) {
    ...
    $scope.$watch("finish.over", function () {
        // 幻灯广告
        Banner.init({
            container: '#oti-top-slider-container',
            effect: 'fade',
            autoplay: 3000,
            pagination: '#oti-top-slide-page'
        });
    });
    ...
    // 获取出境游首页数据
    $http.post(LXB_URL.mOverseasTourIndexData, {}).success(function(result) {
        if (result.success) {
            $scope.result = result;
            // 初始化banner
            $scope.init();
    ...
    $scope.getAreaLineListData = function () {
        ...
        $http.post(LXB_URL.mIndexAreaLineData, {
            ...
            'lineSearchRequest.lineType': 'foreign',
