...
var mDriveTourIndexModule = angular.module('mDriveTourIndexModule', ['infinite-scroll']);
mDriveTourIndexModule.controller('mDriveTourIndexCtrl', function($scope, $http, Banner, TourInit) {
    ...
    $scope.$watch("finish.over", function () {
        // 幻灯广告
        Banner.init({
            container: '#dti-top-slider-container',
            effect: 'fade',
            autoplay: 3000,
            pagination: '#dti-top-slide-page'
        });
    });
    ...
    // 获取自驾游首页数据
    $http.post(LXB_URL.mDriveTourIndexData, {}).success(function (result) {
        if (result.success) {
            $scope.result = result;
            // 初始化banner
            $scope.init();
    ...
    $scope.getAreaLineListData = function () {
        ...
        $http.post(LXB_URL.mIndexAreaLineData, {
            ...
            'lineSearchRequest.productAttr': '自助自驾'
