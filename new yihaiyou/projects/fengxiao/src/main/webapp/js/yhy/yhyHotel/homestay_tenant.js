...
var HotelTenant = {
    hasSend: false,
    init: function() {
        //HotelTenant.initPage();
        HotelTenant.initCom();
        HotelTenant.initValidate();
        //HotelTenant.initAppendice();
    },
    ...
    appendTr: function(index, result) {
        ...
        //html_tr += '<a href="javascript:void(0)"  class="easyui-linkbutton"  onclick="HotelTenant.delTr('+ index +')">删除</a>' ;
        html_tr += '</td>';
        html_tr += '</tr>';
    ...
    addTr: function(result) {
        ...
        HotelTenant.appendTr(index, result);
    },
    ...
    initPage: function() {
        ...
        $.post(url,
            ...
            function(data) {
                if (data.success) {
                    $.loadData({
                        scopeId: '.commpenyMessage',
                        data: data.data
                    });
                    $.loadData({
                        scopeId: '.account_detailUl',
                        data: data.data
                    });
    ...
    getValidateMsgCode: function() {
        ...
        if ($("#newMobileId").val()) {
            ...
            $.post(url, data, function(result) {
                if (result.success) {
                    var second = 60;
                    HotelTenant.hasSend = true;
                    var timer = setInterval(function () {
                        if (second == 0) {
                            $("#sendSms").css("background-color", "#478ff1");
                            $("#sendSms").css("color", "#fff");
                            $("#sendSms").attr("onclick", "HotelTenant.getValidateMsgCode()");
                            $("#sendSms").text("免费获取验证短信");
                            HotelTenant.hasSend = false;
                            clearInterval(timer);
                        } else {
