...
(function (factory) {
    ...
    previewCache = {
        ...
        init: function (obj) {
            ...
            previewCache.data[id] = {
                ...
                actions: function (showUpload, showDelete, showZoom, showDrag, disabled, url, key, homeflag) {	// 20161122图片设置首页功能
                    return obj._renderFileActions(showUpload, showDelete, showZoom, showDrag, disabled, url, key, true, homeflag);
                }
            };
        },
        fetch: function (id) {
        ...
        footer: function (id, i, isDisabled, size) {
            ...
            var config = data.config[i], caption = ifSet('caption', config), width = ifSet('width', config, 'auto'),
                url = ifSet('url', config, false), key = ifSet('key', config, null), homeflag = ifSet('homeflag', config, null),	// 20161122图片设置首页功能
                ...
                actions = data.isDelete ? data.actions(false, showDel, showZoom, showDrag, disabled, url, key, homeflag) : '',
    ...
    FileInput.prototype = {
        ...
        //},
		// 20161122图片设置首页功能
        _initHomeButton: function () {
            var self = this;
            self.$preview.find('.kv-img-home').each(function () {
                var $el = $(this);
                handler($el, 'click', function () {
					if (!$el.length) {
						return;
					}
					var selected = $el.hasClass('kv-home-selected');
					var homeflag = false;
					if (selected) {	// 取消设置为封面，否则设置为封面
						homeflag = false;
					} else {
						homeflag = true;
					}
                    // 取消所有封面，再设置该元素是否为封面
                    self.$preview.find('.kv-img-home').removeClass("kv-home-selected").addClass("kv-home-unselected");
                    if (homeflag) {
                        $el.addClass("kv-home-selected");
                    }

					var config = previewCache.data[self.id].config;
					var elKey = $el.attr("data-key");
					for (var i = 0; i < config.length; i++) {
                        config[i].homeflag = false;
                        if (config[i].extra) {
                            config[i].extra.coverFlag = false;
                        } else {
                            config[i].extra = {coverFlag : false};
                        }
						if (elKey == config[i].key) {
							config[i].homeflag = homeflag;
                            config[i].extra.coverFlag = homeflag;
						}
					}
                });
            });
        ...
        _initPreviewActions: function () {
			...
			// 20161122图片设置首页功能
            if (self.$element.attr('readonly') != 'readonly') {
                self._initHomeButton();
            }
        ...
        _renderFileActions: function (showUpload, showDelete, showZoom, showDrag, disabled, url, key, isInit, homeflag) {	// 20161122图片设置首页功能
            ...
            // 20161122图片设置首页功能
            var vHomeClass = '';
            if (key) {  // 已上传图片，可设置封面，否则不显示封面按钮
                vHomeClass = 'kv-img-home glyphicon glyphicon-home ';
                vHomeClass += homeflag ? 'kv-home-selected' : 'kv-home-unselected';
            }
            if (showDelete) {
            ...
            return template.replace(/\{delete}/g, btnDelete)
                ...
                .replace(/\{dataKey}/g, vKey)// 20161122图片设置首页功能
                .replace(/\{homeClass}/g, vHomeClass);
        },
        _browse: function (e) {
            var self = this;
        ...
        getPreviewConfig: function () {
            var self = this;
            return previewCache.data[self.id].config;
        },
