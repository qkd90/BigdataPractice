            ...
            <table id="qryCondition">
                <tr>
                    ...
                    <td>
                        ...
                        <%--</select>--%>
                        <%--<select class="easyui-combobox" name="city" id="qryCity" style="width:120px">--%>
                            <%--<option value="" selected="selected"></option>--%>
                        <%--</select>--%>
                    </td>
                    <td width="40" align="right">名称:</td>
                    <td><input class="easyui-textbox" id="qry_name" name="name"
                               data-options="prompt:'请输入经验区域的名称'" style="width:200px;"></td>
                    <td></td>
                    <td></td>
                    <td class="pd5">
                    ...
                    <%--<td class="pd5">--%>
                        <%--<a href="javascript:void(0)" class="easyui-linkbutton" style="width:120px"--%>
                           <%--onClick="if(confirm('确定要进行重算吗？针对谷歌坐标系,如港澳台景点')){reCompute()}">重算(*谷歌坐标系)</a>--%>
                    <%--</td>--%>
                    <td class="pd5">
                        <a href="#" class="easyui-linkbutton" style="width:120px"
                           onclick="add()">新增(百度地图)</a>
                    </td>
                    <td class="pd5">
                        <a href="#" class="easyui-linkbutton" style="width:120px"
                           onclick="addAmap()">新增(高德地图)</a>
                    </td>
                </tr>
            </table>
        </div>
...
<div id="scenicsWindow" class="easyui-window" style="width:770px;height: auto"
...
//        $('#scenicsWindow').window('close');
        ...
        $('#relateFather').hide();
        $('#showRelate').show();
        $.get("/region/region/listScenics.jhtml?areaid=" + areaid, function (result) {
            var names = '';
            var sum = 0;
            $.each(result.rows, function (i, n) {
                names += '<input style="display:none" name="childIds" type="checkbox" value="' + n.scenicInfo.scenicIds + '">' +
                        "<a href='javascript:void(0)' style='color:blue;text-decoration: underline;' onclick='editScenic(" + n.scenicInfo.id + ")'>" + n.scenicName + "</a>&nbsp;";
                if (sum++ > 6) {
                    names += '</br>';
                    sum = 1;
                }
            ...
            $('#fatherSubmit').click(function () {
                $('#relateForm').form('submit', {
                    url: '/region/region/saveRes.jhtml',
                    dataType: 'json',
...
//        $('#qryCity').combobox('select', null);
    }
    function reCompute() {
        var cityCode = $('#qry_cityCode').val();
//        var cityCode = $('#qryCity').attr("data-city");
//        if (!cityCode) {
//            cityCode = $('#qryCity').attr("data-province");
//            if (!cityCode) {
//                cityCode = $('#qryCity').attr("data-country");
//            }
//        }
        if (!cityCode || cityCode == '') {
            ...
            $.messager.confirm('重算确认', '重算需要花费较长时间, 确认继续?', function(r) {
                if (r) {
                    ...
                    $.ajax({
                        url: '/region/region/recompute.jhtml',
                        type: 'post',
                        dataType: 'json',
                        data: {
                            cityCode: cityCode
    ...
    // 新增
    function add() {
        window.parent.addTab('新增区域', "/region/region/add.jhtml");
    }
    // 新增
    function addAmap() {
        window.parent.addTab('新增高德区域', "/region/region/amapAdd.jhtml");
    }
    // 修改
    function edit(id) {
        window.parent.addTab('编辑区域', "/region/region/edit.jhtml?id=" + id);
    }
    // 修改
    function editAmap(id) {
        window.parent.addTab('编辑区域', "/region/region/amapEdit.jhtml?id=" + id);
    }
    // 修改
    ...
    function del(id, name) {
        ...
        if (sure) {
            $.get("/region/region/del.jhtml?id=" + id);
            $("#qryResult").datagrid("load", {});
        }
    }
    // 页面加载后相关设置项
...
//            onLoadSuccess: init
    ...
    function optFormatter(value, rowData) {
        var btnEdit = "<a href='javascript:void(0)' style='color:blue;text-decoration: underline;' onclick='edit(" + rowData.id + ")'>编辑(百度)</a>";
        btnEdit += "&nbsp;<a href='javascript:void(0)' style='color:blue;text-decoration: underline;' onclick='editAmap(" + rowData.id + ")'>编辑(高德)</a>";
//        btnEdit += "&nbsp;<a href='javascript:void(0)' style='color:blue;text-decoration: underline;' onclick='listScenics(" + rowData.id + ",\"" + rowData.name + "\")'>景点</a>";
        btnEdit += "&nbsp;<a href='javascript:void(0)' style='color:blue;text-decoration: underline;' onclick='del(" + rowData.id + ",\"" + rowData.name + "\")'>删除</a>";
        return btnEdit;
    }
    function regionTypeFormat(value, rowData, rowIndex) {
        if (value == 1) {
            return "百度";
        } else if (value == 2) {
            return "高德";
