 ...
 */
public class DeployUtilForBg {
    private static Log log = LogFactory.getLog(DeployUtilForBg.class);
    private static final String PROJECT_NAME = "fengxiao";
    private static final String TARGET_PROJECT_NAME = PROJECT_NAME + "-0.0.1-SNAPSHOT";
    private static String WORKSPACE_DIR = null;
    private static final String DEPLOY_NAME = "bg-webapp";

    public static void main(String[] args) throws IOException, InterruptedException {
        // 设置工作路径
        String currentUPath = DeployUtilForBg.class.getClass().getResource("/").getPath();
        ...
        */
        ...
        // 执行批处理，文件替换和覆盖
        cmd = deployDir.substring(1) + "bg.bat";
        ...
        // kindeditor plugins不能带版本号，删除有版本号的文件，重新复制
        // 删除有版本的文件
        String kindeditorPluginsTargetPath = deployDir + "js/kindeditor/plugins";
        File kindeditorPluginsVerDir = new File(kindeditorPluginsTargetPath);
        if (kindeditorPluginsVerDir.exists()) {
            FileUtils.deleteDir(kindeditorPluginsVerDir);
        }
        String kindeditorPluginsSourcePath = dir + "js/kindeditor/plugins";
        // 复制原文件
        FileUtils.copyDir(kindeditorPluginsSourcePath, kindeditorPluginsTargetPath);
