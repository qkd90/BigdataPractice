...
var SailboatFinance = {
    ...
    init: function() {
        SailboatFinance.initPageComm();
        SailboatFinance.initOrderSummaryListTable();
    },
    ...
    initOrderSummaryListTable: function() {
        SailboatFinance.orderSummaryTable = $("#orderFinanceId").DataTable( {
            "serverSide": true,
            "searching": false,
            "ordering": false,
            ...
            columns: [
                ...
                { "title": "结算方式", data:'billType', "class": "center" , render: function(data, type, row, meta) {
                ...
                { "title": "操作", data:null, render: function(data, type, row, meta) {
                    return '<a onclick="SailboatFinance.orderBillSummaryDetail('+ row.id +')">账单明细</a>';
                }}
            ]
        });
    },

    getOrderSummaryList: function(sSource, aoData, fnCallback) {
        var data = {};
        ...
        if($("#billOrderSummaryNo").val()) {
            data['orderBillSummary.billNo'] = $("#billOrderSummaryNo").val();
        }
    ...
    search: function() {
        SailboatFinance.orderSummaryTable.ajax.reload();
    },
    orderBillSummaryDetail: function(id) {
        window.location.href = "/yhy/yhyMain/toSailboatStatment.jhtml?orderBillSummary.id="+id;
    }

}

$(function() {
    SailboatFinance.init();
});
