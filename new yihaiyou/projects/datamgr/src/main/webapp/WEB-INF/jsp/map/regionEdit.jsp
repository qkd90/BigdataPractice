...
<form id="region_edit_form" action="/region/region/saveRegion.jhtml" method="post" enctype="multipart/form-data">
    ...
    <div>
        ...
        前
        <input type="text" class="easyui-numberspinner" data-options="min:0,required:true" id="ranking" value="99999"/>
        名
        <input type="button" value="加载" class="easyui-linkbutton" style="padding: 3 10;" onclick="loadCity()"/>
        地址<input type="text" id="address" class="easyui-textbox" value=""/>
        <input type="button" value="查询" class="easyui-linkbutton" style="padding: 3 10;" onclick="searchByStationName()"/>
    </div>
    <div>
        名称：<input id="name" name="name" value="${region.name}" data-options="required:true"/>
        优先级：<input id="priority" name="priority" class="easyui-numberspinner" data-options="min:0,required:true"
                   value="${region.priority}"/>
        <input type="button" value="清空" class="easyui-linkbutton" style="padding: 3 10;" onclick="clean()"/>
        <input type="button" value="提交" class="easyui-linkbutton" style="padding: 3 10;" onclick="save()"/>
        ...
        <input type="hidden" id="id" name="id" class="easyui-textbox" value="${region.id==null?0:region.id}"/>
        &nbsp;&nbsp;
        定位景点:
        <span>
            <input type="text" id="fatherSelect" value="" class="easyui-combobox"
                   data-options="loader: scenicLoader,mode: 'remote',valueField: 'scenicId',textField: 'scenicName',onSelect:selectScenic">
            </span>
    </div>
...
//        init();
    ...
    function save() {
        $('#region_edit_form').form('submit', {
            url: '/region/region/saveRegion.jhtml',
...
//                        map.addOverlay(new BMap.Marker(new BMap.Point(n.longitude, n.latitude)));
                        ...
                        var info = n.address + "</br><a style='color: #00008B' href='#' onclick='editScenic(" + n.scenicId + ")'>点此查看详情</a>";
