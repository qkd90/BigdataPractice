var PlanScenicList = {
    ...
    mapIcon: null,
    init: function () {
        PlanScenicList.initComp();
        PlanScenicList.initEvt();
        PlanScenicList.createPager();
        PlanScenicList.initData();
    },
    initComp: function() {
        // init BMap
        PlanScenicList.map = new BMap.Map("r_map");
        PlanScenicList.map.centerAndZoom("%E5%8E%A6%E9%97%A8", 11);
        PlanScenicList.mapIcon = new BMap.Icon("/image/places-16.png", new BMap.Size(20, 25));
    },
    initEvt: function () {
        PlanScenicList.scenicSelect('#area li');//区域选择
        PlanScenicList.scenicSelect('#theme li');//主题选择
        PlanScenicList.scenicSelect('#price li');//价格选择
        PlanScenicList.openlist();
        PlanScenicList.searchSelect();
        PlanScenicList.mapstick();
        // customize price btn
        $('#custom_price_btn').on('click', function(event) {
            ...
            PlanScenicList.getScenicListData();
        });
        // order btn
        ...
        $('#clear_filter_btn').on('click', function(event) {
            event.stopPropagation();
            PlanScenicList.resetSearchData();
        });
    ...
    createPager: function () {
        var options = {
            ...
            pageRenderFn: function (pageNo, pageSize, data) {
                var planScenicList = $("#scenicList");
                planScenicList.empty();
        ...
        PlanScenicList.pager = new Pager(options);
    },

    getScenicListData: function () {
        var searchRequest = PlanScenicList.buildSearchData();
        PlanScenicList.pager.init(searchRequest);
    },
    ...
    buildSearchData: function() {
        ...
        if (searchWord && searchWord != "") {
            ...
            PlanScenicList.displayFilter(null, 'searchWord', searchWord);
        ...
        $.each($areaSel, function(i, a) {
            ...
            PlanScenicList.displayFilter(null, 'area', $.trim($(a).html()));
        });
        // theme labels
        ...
        $.each($themeSel, function(i, t) {
            ...
            PlanScenicList.displayFilter(lid, 'theme', $.trim($(t).html()));
        });
        // price
        ...
        if ($priceSel.length > 0) {
            if ($priceSel.attr('data-price-mode') == "range") {
                ...
                PlanScenicList.displayFilter(null, "price", $.trim($priceSel.html()));
            } else if ($priceSel.attr('data-price-mode') == "customize") {
                var minPrice = $('#custom_min_price').val();
                var maxPrice = $('#custom_max_price').val();
                searchRequest['searchRequest.priceRange[0]'] = minPrice;
                searchRequest['searchRequest.priceRange[1]'] = maxPrice;
                PlanScenicList.displayFilter(null, "price", minPrice + "-" + maxPrice + "元");
            }
        }
