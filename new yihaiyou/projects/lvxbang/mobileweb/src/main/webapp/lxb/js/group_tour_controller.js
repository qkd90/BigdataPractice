...
var mGroupTourIndexModule = angular.module('mGroupTourIndexModule', ['infinite-scroll']);
mGroupTourIndexModule.controller('mGroupTourIndexCtrl',function($scope, $http, Banner, TourInit) {
    ...
    $scope.$watch("finish.over", function () {
        // 幻灯广告
        Banner.init({
            container: '#gti-top-slider-container',
            effect: 'fade',
            autoplay: 3000,
            pagination: '#gti-top-slide-page'
        });
    });
    ...
    // 获取跟团游首页数据
    $http.post(LXB_URL.mGroupTourIndexData, {}).success(function(result) {
        if (result.success) {
            $scope.result = result;
            // 初始化banner
            $scope.init();
    ...
    $scope.getAreaLineListData = function () {
        ...
        $http.post(LXB_URL.mIndexAreaLineData, {
            ...
            'lineSearchRequest.productAttr': '跟团游'
